...
data = scipy.io.loadmat('./data/mat_data/gen_3_data.mat')

t = data['t'].flatten()[:,None]
x3 = data['x3'].flatten()[:,None]
...
Exact = np.real(data['usol3'])

lb = np.array([t_min, x3_min])
ub = np.array([t_max, x3_max])

X_star = np.hstack((t, x3))
...
u_pred_df.to_excel('./results/gen_3.xlsx')